...
def tree():
    ...
    (train_set1, test_set1) = first_divide_dataset.first_divide_dataset(groups1)
    groups2 = [[0, 1], [2, 3, 4]]
    (train_set2, test_set2) = prepare_dataset.prepare_dataset(groups2)
    groups3 = [[5, 6], [7, 8, 9]]
    (train_set3, test_set3) = prepare_dataset.prepare_dataset(groups3)
    groups4 = [[0], [1]]
    (train_set4, test_set4) = prepare_dataset.prepare_dataset(groups4)
    groups5 = [[2], [3]]
    (train_set5, test_set5) = prepare_dataset.prepare_dataset(groups5)
    groups6 = [[3], [4]]
    (train_set6, test_set6) = prepare_dataset.prepare_dataset(groups6)
    groups7 = [[5], [6]]
    (train_set7, test_set7) = prepare_dataset.prepare_dataset(groups7)
    groups8 = [[7], [8]]
    (train_set8, test_set8) = prepare_dataset.prepare_dataset(groups8)
    groups9 = [[8], [9]]
    (train_set9, test_set9) = prepare_dataset.prepare_dataset(groups9)
    groups10 = [[2, 3], [4]]
    ...
    node7.train(1)
    node6 = Node.Node(train_set7, test_set7, leaf56a, leaf56b, './node6_net.pth', False)
    node6.train(1)
    node5 = Node.Node(train_set10, test_set10, leaf23, leaf34, './node5_net.pth', False)
    node5.train(1)
    node4 = Node.Node(train_set4, test_set4, leaf01a, leaf01b, './node4_net.pth', False)
    node4.train(1)
